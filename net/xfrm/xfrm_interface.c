...
static struct xfrm_if *xfrmi_create(struct net *net, struct xfrm_if_parms *p)
{
	...
	dev = alloc_netdev(sizeof(*xi), name, xfrmi_dev_setup);
	if (!dev)
		goto failed;
...
static int xfrmi6_err(struct sk_buff *skb, __be32 info)
{
...
static struct xfrm_tunnel xfrm4_handler __read_mostly = {
	.handler	=	xfrm4_rcv,
	.err_handler	=	xfrmi4_err,
	.priority	=	10,
};

static struct xfrm_tunnel xfrm6_handler __read_mostly = {
	.handler	=	xfrm6_rcv,
...
static int __init xfrmi_init(void)
{
	...
	msg = "xfrm4 tunnel";
	err = xfrm4_tunnel_register(&xfrm4_handler, AF_INET);
	if (err < 0)
		goto xfrmi4_failed;

	msg = "xfrm6 tunnel";
	err = xfrm4_tunnel_register(&xfrm6_handler, AF_INET6);
	if (err < 0)
		goto xfrmi6_failed;
...
rtnl_link_failed:
	xfrm4_tunnel_deregister(&xfrm6_handler, AF_INET6);
xfrmi6_failed:
	xfrm4_tunnel_deregister(&xfrm4_handler, AF_INET);
xfrmi4_failed:
