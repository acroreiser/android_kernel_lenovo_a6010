...
static bool qtaguid_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (sk != NULL) {
		...
		MT_DEBUG("qtaguid[%d]: filp...uid=%u\n",
			par->hooknum, filp ? filp->f_cred->fsuid.val : -1);
	}
	...
	if (info->match & XT_QTAGUID_UID)
		if ((filp->f_cred->fsuid.val >= info->uid_min &&
		     filp->f_cred->fsuid.val <= info->uid_max) ^
	...
	if (info->match & XT_QTAGUID_GID)
		if ((filp->f_cred->fsgid.val >= info->gid_min &&
				filp->f_cred->fsgid.val <= info->gid_max) ^
...
static int ctrl_cmd_delete(const char *input)
{
	...
	if (argc < 3) {
		...
		pr_info("qtaguid: ctrl_delete(%s): "
			...
			input, current->pid, current->tgid, current_fsuid().val);
		res = -EPERM;
		goto err;
	}
...
static int ctrl_cmd_tag(const char *input)
{
	...
	CT_DEBUG("qtaguid: ctrl_tag(%s): "
		 ...
		 input, current->pid, current->tgid, current_uid().val,
		 current_euid().val, current_fsuid().val,
		 xt_qtaguid_ctrl_file->gid.val,
...
static int qtudev_open(struct inode *inode, struct file *file)
{
	...
	DR_DEBUG("qtaguid: tracking data for uid=%u in pqd=%p\n",
		 current_fsuid().val, new_pqd_entry);
