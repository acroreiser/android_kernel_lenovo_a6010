...
static int read_cpudata(cpudata_t * cpudata, int core)
{
	...
	ret = fscanf(fp, "cpu%d %llu %llu %llu %llu %llu %llu %llu %llu %llu %llu\n", &coredummy,
				 ...
				 &cpudata->cputime_steal, &cpudata->cputime_guest, &cpudata->cputime_guest_nice);

	if (ret != 11)
	{
		if (cpudata->online)
		{
			printf("%s: --- marking cpu%d as offline\n", __func__, core);
	...
	if (!cpudata->online)
		printf("%s: +++ marking cpu%d as online\n", __func__, core);
	...
	for (i = 0; i < NR_CPUS; i++)
	{
		...
		if (ret)
		{
			if (__cpudata[i].online)
				printf("%s: failed to read cpudata for core %d: %s (%d)", __func__, core,
					   strerror(-ret), -ret);
			break;
		}
...
#define TOTAL_CPUTIME(_data) \
		...
		printf("%u / %u\n", cputime_delta, cputime_total_delta);
		if (cputime_total_delta == 0)
			cputime_total_delta = 1;
		(&__cpudata[i])->cpu_util = (cputime_delta * 100) / cputime_total_delta;
		printf("%s: +++ core%d: util=%llu (%llu/%llu)\n", __func__, core, __cpudata[i].cpu_util,
			   cputime_delta, cputime_total_delta);
	}
}
...
static void irqbalance_main()
{
	...
	for (i = NR_CPUS - 1; i >= 0 && irqs_processed < irqs_num; i--)
	{
		...
		for (j = 0; j < irqnum && irqs_processed < irqs_num; j++)
		{
			...
			{
				printf("%s: --- failed to open IRQ SMP affinity file for IRQ %d\n", __func__,
					   irqs[irqidx]);
			}
		}
...
static int read_irqbalance_configuration(void)
{
	...
	while (!feof(fp))
	{
		...
		else if (!strcmp(cmd, "blck"))
		{
			...
			irq_blacklist = (int *)malloc(irq_blacklist_sz);
			if (!irq_blacklist)
			{
				printf("%s: could not allocate memory for IRQ blacklist (%d bytes)\n", __func__,
					   irq_blacklist_sz);
				return -ENOMEM;
			}
			...
			for (i = 0; i < irq_blacklist_num; i++)
			{
				...
				{
					printf
						("%s: could not read given number of blacklisted IRQs: given %d, read %d\n",
		...
		{
			printf("%s: parsed invalid command: '%s'\n", __func__, cmd);
			return -EINVAL;
		}
	}
...
static int scan_for_irqs(void)
{
	...
	if (!irqs)
	{
		printf("%s: could not allocate memory for IRQ list (%d bytes)\n", __func__, sz);
		return -ENOMEM;
	}
	...
	for (i = 0; i < irqs_num;)
	{
		...
		for (j = 0; j < irq_blacklist_num; j++)
		{
			...
			{
				printf("%s: --- Skipping IRQ %d: blacklisted\n", __func__, irqs[i]);
				goto skipirq;
			}
		}
...
int main(int argc, char *argv[])
{
	...
	printf("%s: initializing irqbalance configuration\n", __func__);
	...
	printf("%s: allocating memory for CPU data\n", __func__);
	dtsz = sizeof(cpudata_t) * NR_CPUS;

	__cpudata = (cpudata_t *) malloc(dtsz);
	if (!__cpudata)
	{
		printf("%s: could not allocate memory for CPU data (%d bytes)\n", __func__, dtsz);
		return -ENOMEM;
	}

	memset(__cpudata, 0, dtsz);

	printf("%s: initializing CPU data\n", __func__);
	for (i = 0; i < NR_CPUS; i++)
	{
		...
		if (ret)
		{
			printf("%s: fail to read initial cpudata for core %d", __func__, i);
			return ret;
		}
	}

	printf("%s: starting IRQ balancing loop\n", __func__);
	switch (pid = fork())
	{
	case -1:
	...
	printf("%s: reached unreachable code??\n", __func__);
	return 0;
}