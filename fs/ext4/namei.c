...
static struct buffer_head * ext4_dx_find_entry(struct inode *dir,
		...
		int *err);
static int ext4_dx_add_entry(handle_t *handle, struct ext4_filename *fname, struct dentry *dentry,
			     struct inode *inode);
					...
					int *inlined)
{
	...
	*res_dir = NULL;
	...
	if (is_dx(dir)) {
		ret = ext4_dx_find_entry(dir, &fname, res_dir, &err);
		/*
		 * On success, or if the error was file not found,
		 * return.  Otherwise, fall back to doing a search the
		 * old fashioned way.
		 */
		if (ret || (ret != ERR_BAD_DX_DIR))
			goto cleanup_and_exit;
...
static struct buffer_head * ext4_dx_find_entry(struct inode *dir, struct ext4_filename *fname,
		       struct ext4_dir_entry_2 **res_dir, int *err)
{
	...
	*res_dir = NULL;
#endif
	if (!(frame = dx_probe(fname, dir, NULL, frames, err)))
		return NULL;
	do {
		block = dx_get_block(frame->at);
		bh = ext4_read_dirblock(dir, block, DIRENT);
			    ...
			    struct inode *inode, struct buffer_head *bh)
{
	...
	de = do_split(handle,dir, &bh, frame, &fname->hinfo, &retval);
	if (!de) {
		/*
			     ...
			     struct dentry *dentry, struct inode *inode)
{
	...
	frame = dx_probe(fname, dir, NULL, frames, &err);
	if (!frame)
		return err;
	...
	de = do_split(handle, dir, &bh, frame, &fname->hinfo, &err);
	if (!de)
		goto cleanup;
		       ...
		       struct inode *new_dir, struct dentry *new_dentry)
{
	...
	if (dir_bh) {
		...
		if (new_inode) {
			/* checked ext4_empty_dir above, can't have another parent,
			 * ext4_dec_count() won't work for many-linked 
			 * dirs */
