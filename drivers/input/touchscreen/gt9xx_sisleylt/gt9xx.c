...
//ahe change 
//static u8 gtp_fw_version_num ;
static int goodix_power_on(struct goodix_ts_data *ts, bool on);
static int gtp_pinctrl_select(struct goodix_ts_data *info,bool on);
static s8 gtp_request_io_port(struct goodix_ts_data *ts,bool on);
static void gtp_resume_flow(struct work_struct *work);
...
static int letter = 0 ;
//static struct work_struct gtp_doze_work_queue;
void gtp_doze_work_queue_fun(struct work_struct *work);
#endif
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
    ...
    if((retries >= 5))
    {
        ...
        if (DOZE_DISABLED!= doze_status && gtp_wakeup_flag)//ahe change 
        {
            return ret;
        }
    #endif
	GTP_ERROR("ahe .doze_status %d,gtp_wakeup_flag :%d",doze_status,gtp_wakeup_flag);
...
void gtp_irq_enable(struct goodix_ts_data *ts)
{
    ...
    if(ts->disable_gpios){
    	GTP_INFO("gtp_irq_enable cancel because of disable_gpios");
   	return;
    }
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
    ...
    GTP_SWAP(x, y);
#endif
	//GTP_DEBUG("Touch id[%2d] down!", id);
#if GTP_ICS_SLOT_REPORT
    input_mt_slot(ts->input_dev, id);
    input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, id);
    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, x);
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, y);
    input_report_abs(ts->input_dev, ABS_MT_PRESSURE, w);//ahe
...
static void goodix_ts_work_func(struct goodix_ts_data *ts)
{
    ...
    //GTP_ERROR("~~~~ahe touch_num :%d   key_value :%02x",touch_num,key_value);
...
#if GTP_GESTURE_WAKEUP
static void goodix_gesture_work_func(struct work_struct *work)
{

    u8 doze_buf[3] = {0x81, 0x4B};
    u8 tap_type[3] = {0x81, 0x4d};
	int ret ;
    struct goodix_ts_data *ts = NULL;
     struct delayed_work* dwork = to_delayed_work(work);
    GTP_DEBUG_FUNC();
    ts = container_of(dwork, struct goodix_ts_data, work); 
    if ((gtp_wakeup_flag)&&(DOZE_DISABLED!= doze_status))
    {  
		...
		if(type == 0){
            ...
            {
        		ret = gtp_i2c_read(i2c_connect_client, tap_type, 3);
               	 doze_status = DOZE_WAKEUP;
		if(tap_type[2] == 2){	
	...
	*/
		...
		else if(tap_type[2] == 1||tap_type[2] == 4){
			//double click menu and back key and  wo do nothing!!
		}
		else{
 ...
 // clear 0x814B
		...
		GTP_INFO("Gesture  I2C Read fail  !!! !");
        	}

        if (ts->use_irq)
        {
            //GTP_INFO("ahe work gtp_irq_enable 11");
            gtp_irq_enable(ts);
        }
        return;
    }
...
static enum hrtimer_restart goodix_ts_timer_handler(struct hrtimer *timer)
{
    ...
    GTP_DEBUG_FUNC();
	 GTP_INFO("goodix_ts_timer_handler.");
	/*
...
static irqreturn_t goodix_ts_irq_handler(int irq, void *dev_id)
{
   ...
   //GTP_INFO("goodix_ts_irq_handler.");

    GTP_DEBUG_FUNC();
    gtp_irq_disable(ts); 
    if(gtp_wakeup_flag && (doze_status != DOZE_DISABLED))
    {
    	GTP_INFO("goodix_ts_irq_handler g");
    	queue_delayed_work(goodix_wq, &ts->work,5);//ahe change 
    }
    else
    	//queue_delayed_work(goodix_wq, &ts->work,0);
...
void gtp_int_sync(struct goodix_ts_data *ts, s32 ms)
{
	...
	//GTP_INFO("ahe set  irq_gpio as input\n");
}
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
	...
	//GTP_INFO("*********************************  reset  0 ");
	gpio_direction_output(ts->pdata->irq_gpio, 0);//ahe change
	msleep(ms);
	if (ts->client->addr == GTP_I2C_ADDRESS_HIGH)
		gpio_direction_output(ts->pdata->irq_gpio, 1);
	else{
		gpio_direction_output(ts->pdata->irq_gpio, 0);
		//GTP_INFO("-------------------------: %d",gpio_get_value(ts->pdata->irq_gpio));
	}
...
#endif
/*lenovo-sw2,change for gtp_tool abnormal exit */
if(ts->gtp_rawdiff_mode){
    	ts->gtp_rawdiff_mode = false;
	GTP_DEBUG("ahe set false to make sure clear int flagt");
}
/*lenovo-sw2,change for gtp_tool abnormal exit */
}
...
void gtp_doze_work_queue_fun(struct work_struct *work)
{
	struct goodix_ts_data *ts = container_of(work, struct goodix_ts_data, gtp_enter_doze_work);
    int i = 0;
    if(doze_status == DOZE_ENABLED){
        GTP_DEBUG("ahe quit re_enter  \n"); 
        return;
    }
    ...
    for(i=0;i<10;i++){
        ...
        if(doze_status==DOZE_DISABLED){
            GTP_DEBUG("ahe Gesture wake_up successful \n"); 
            break;	
        }else{	
            if(i==9) {	
               GTP_DEBUG("ahe gesture wake_up is cancel,re_enter doze mode "); 
...
static s8 gtp_enter_sleep(struct goodix_ts_data * ts)
{
    ...
    while(retry++ < 5)
    {
        ...
        if (ret > 0)
        {
            GTP_INFO("GTP enter sleep retry :%d !",(retry-1));
            break ;
           // return ret;
        }
        msleep(10);
...
#endif
	if(ts->powerdown == false){
		//GTP_INFO("GTP power down!");
		ret = goodix_power_on(ts,false);
		if (ret) {
			dev_err(&ts->client->dev, "GTP power down failed\n");
			goto err_lpm_regulator;
		}
		ts->powerdown = true ;
		GTP_INFO("power_disable");
	}
	if(ts->disable_gpios==false){
		
		ret = gtp_pinctrl_select(ts, false);
		ret = gtp_request_io_port(ts,false);
		if (ret < 0){
			dev_err(&ts->client->dev, "GTP enter_sleep request_io_port failed\n");
			goto err_gpio_configure;
		}
		ts->disable_gpios= true ;
		GTP_INFO("gpio_disable");
	}
	return ret;
err_lpm_regulator:
...
err_gpio_configure:
	if (ts->ts_pinctrl) {
		ret =gtp_pinctrl_select(ts, true);
		if (ret < 0)
			dev_err(&ts->client->dev, "GTP enter_sleep request_io_port failed\n");
	}
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
	...
	if(gtp_wakeup_flag && (doze_status==DOZE_ENABLED)){
        	if (DOZE_WAKEUP != doze_status)
      		...
      		gtp_irq_disable(ts);//ahe changed 
        	        gtp_reset_guitar(ts->client, 10);
       		// gtp_irq_enable(ts);
	}else{
		GTP_DEBUG("%s,powerkey wakeup",__func__);
		if(ts->powerdown == true){
			ret = goodix_power_on(ts,true);
			if (ret) {
				dev_err(&ts->client->dev, "GTP wake up power on failed\n");
				return ret ;
			}
			ts->powerdown = false ;
		}
		if(ts->disable_gpios==true){
			ret = gtp_pinctrl_select(ts, true);
			if (ret < 0){
				dev_err(&ts->client->dev, "GTP wake up pnictrl select  failed\n");
				//goto exit_power_off;
			}
    			ret = gtp_request_io_port(ts,true);
			if (ret < 0){
				dev_err(&ts->client->dev, "GTP wake up  request I/O  failed\n");
				goto err_gpio_configure;
			}
			ts->disable_gpios= false ;
		//gtp_reset_guitar(ts->client, 20); //ahe change to resume faster
        	//GTP_GPIO_OUTPUT(ts->pdata->irq_gpio, 1);
       		 //msleep(5);
		}
	}
...
err_gpio_configure:
	if (ts->ts_pinctrl) {
		ret = gtp_pinctrl_select(ts, false);
		if (ret < 0)
			dev_err(&ts->client->dev,"ahe Cannot get idle pinctrl state\n");
	}
...
static int gtp_pinctrl_init(struct goodix_ts_data *info)
{
	int retval;
	...
	gtp_reset_guitar(ts->client,20);
		...
		if (ts->disable_gpios ==false) {
			if (gpio_is_valid(pdata->irq_gpio)){
				...
				//dev_err(&client->dev, "enter sleep free irq_gpio  !\n");
			}
			if (gpio_is_valid(pdata->reset_gpio)) {
 			/*
			...
			//dev_err(&client->dev, "free gpio is 22222!\n");
		}
	}
	return ret;
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    input_set_abs_params(ts->input_dev, ABS_MT_PRESSURE, 0, 1, 0, 0);//ahe
...
power_off:
    ...
    //if(gtp_fw_version_num > 0x42){//pvt close L6
       // printk("ahe num: %02x\n",gtp_fw_version_num);
	    if (!IS_ERR(ts->vcc_i2c)) {
...
*/
	...
	for (i = 0; i < GOODIX_MAX_CFG_GROUP; i++) {
		...
		//prop = of_find_property(np, prop_name,&pdata->config_data_len[i]);//remove by lixh10
		if (!prop || !prop->value) {
		...
		struct device_attribute *attr, char *buf)
{
	...
	return snprintf(buf, PAGE_SIZE, "gt%s_%s_%s %02x\n",GTP_IC_VERSION,vendor_id,GTP_FW_VERSION,grp_cfg_version);
}
#if GTP_GESTURE_WAKEUP 
...
static ssize_t mms_test_fluency(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	int count ;
	int startpoint;
	struct goodix_ts_data *info = dev_get_drvdata(dev);
	if(info->gtp_is_suspend)
		return 0;
	j = 10 ;
	test_x =	150 ;//10 ;// LCD_Y/2 ;  //160--680
	startpoint = test_x;
	test_y = 427 ;//LCD_Y/2;
	test_w = 30;
         test_id = 0;
	  count = (680-160)/coordinate_interval ;
	  gtp_irq_disable(info);
...
#endif
		gtp_touch_down(info,test_id,test_x,test_y,test_w);
		  input_sync(info->input_dev);
...
static struct device_attribute attrs[] = {
	__ATTR(lenovo_tpd_info, 0664,
			gtp_information_show,
			NULL),
#if GTP_GESTURE_WAKEUP
	__ATTR(tpd_suspend_status, 0664,
			gtp_gesture_wakeup_show,
			gtp_gesture_wakeup_store),
	__ATTR(lenovo_flag, 0664,
			mms_gesture_flag_show, 
			NULL),
#endif
#ifdef LENOVO_CTP_TEST_FLUENCY
	__ATTR(test_fluency_delay, 0664,
			mms_test_fluency_delay_show,
			mms_test_fluency_delay_store),
	__ATTR(test_fluency, 0664,mms_test_fluency, NULL),
	__ATTR(test_fluency_intval, 0664,
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    INIT_DELAYED_WORK(&ts->work, goodix_gesture_work_func );
#if GTP_GESTURE_WAKEUP
    INIT_WORK(&ts->gtp_enter_doze_work, gtp_doze_work_queue_fun);
#endif
#ifdef GTP_CUT_RESUME_TIME
    INIT_WORK(&ts->gtp_resume_work, gtp_resume_flow);
#endif
#ifdef LENOVO_CTP_TEST_FLUENCY
   ...
   // spin_lock_init(&ts->esd_lock);
    ...
    ret = gtp_request_io_port(ts,true);
    ts->irq_is_disable = 0;// ahe init irq status 
   dev_err(&client->dev, "ahe gtp_request_io_port done \n");
    if (ret < 0)
    {
  ...
  //  gtp_fw_version_num = cfg_info_group1[0];
    if (ret < 0)
    {
...
//	msleep(30000);
	...
	if (sysfs_create_link(NULL, &ts->dev->kobj, "board_properties")) {
		dev_err(&client->dev, "failed to create sysfs symlink\n");
		return -EAGAIN;
	}
	
	dev_set_drvdata(ts->dev,ts);
	dev_err(&client->dev,"probe done !");
...
#if 0
static int  goodix_ts_power_ctrl(struct goodix_ts_data *ts,bool status)
{
	int ret ; 
	...
	if(true==status){
		ret = goodix_power_on(ts, true);
		if (ret) {
			dev_err(&ts->client->dev, "ahe failed to do resume power on \n");			
		}
		}else{
			ret =goodix_power_on(ts, false);
			if (ret ) {
				dev_err(&ts->client->dev,, "ahe failed to do suspend power off\n");
				goto err_lpm_regulator;
			}
		}
err_lpm_regulator:
...
{
	int ret = -1 ;
	struct goodix_ts_data *ts =  container_of(work, struct goodix_ts_data, gtp_resume_work);
    	GTP_INFO("gtp_resume_flow  11.");
    	ret = gtp_wakeup_sleep(ts);
    	GTP_DEBUG("gtp_resume_flow  22.");
#if GTP_GESTURE_WAKEUP
	if(gtp_wakeup_flag)
    		doze_status = DOZE_DISABLED;
#endif
	 ...
	 GTP_DEBUG("gtp_resume_flow 44.");
    	ts->gtp_is_suspend = 0;
#if GTP_ESD_PROTECT
    	gtp_esd_switch(ts->client, SWITCH_ON);
#endif
	 GTP_INFO("gtp_resume_flow 55.");
...
static void goodix_ts_suspend(struct goodix_ts_data *ts)
{
	int ret = -1;
	  if(ts->gtp_is_suspend == 1){
	  	    GTP_INFO("System already suspend ~~~~~~~~");
			return;
	  } 	
	mutex_lock(&ts->lock);
	GTP_DEBUG_FUNC();
	...
	else
	{
    		...
    		ret = gtp_enter_sleep(ts);
#if 0 //yangjq, 20140711, Revert ldo6 and ldo17's turn off for modem issue
	if(!gtp_wakeup_flag)
	{
		ret = goodix_power_on(ts,false);
		if (ret) {
...
{
#ifndef GTP_CUT_RESUME_TIME
	int ret = -1;
#endif
	  if(ts->gtp_is_suspend == 0){
	  	    GTP_INFO("System already resume ~~~~~~~~");
			return;
	  } 	
	  mutex_lock(&ts->lock);
	  GTP_DEBUG_FUNC();
	  GTP_INFO("ahe  resume start.");
#if 0 //yangjq, 20140711, Revert ldo6 and ldo17's turn off for modem issue
...
#endif //0
#ifndef GTP_CUT_RESUME_TIME
    GTP_DEBUG("ahe  resume &&&&&& 111111.");
    ret = gtp_wakeup_sleep(ts);
    GTP_DEBUG("ahe  resume &&&&&& 222222.");
#if GTP_GESTURE_WAKEUP
	if(gtp_wakeup_flag)
    		doze_status = DOZE_DISABLED;
#endif
    ...
    else
    {
	 ...
	 GTP_DEBUG("ahe  resume &&&&&& 444444.");
    ts->gtp_is_suspend = 0;
#if GTP_ESD_PROTECT
    gtp_esd_switch(ts->client, SWITCH_ON);
#endif
	 GTP_DEBUG("ahe  resume &&&&&& 5555555.");
#else
	schedule_work(&ts->gtp_resume_work);
#endif
	mutex_unlock(&ts->lock);
	GTP_INFO("ahe  resume end.");
}
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK &&
		...
		if (*blank == FB_BLANK_UNBLANK){
			GTP_INFO("ahe fb_notifier_callback resume !!!!");
			goodix_ts_resume(ts);
		}
		else if (*blank == FB_BLANK_POWERDOWN){
			GTP_INFO("ahe fb_notifier_callback suspend !!!!");
...
static void goodix_ts_early_suspend(struct early_suspend *h)
{
	...
	GTP_INFO("ahe goodix_ts_early_suspend  !!!!");
...
static void gtp_esd_check_func(struct work_struct *work)
{
    ...
    if (i >= 3)
    {
    ...
    #endif
        {
            ...
            gtp_i2c_write_no_rst(ts->client, esd_buf, 5);
            msleep(50);
	  gtp_irq_disable(ts);//ahe change 
            gtp_reset_guitar(ts->client, 50);
            msleep(50);//ahe changed
	  ret = gtp_i2c_test(ts->client);
	  if(ret>0){	 
	          gtp_irq_enable(ts);//ahe change 
           	 gtp_send_cfg(ts->client);
	  }
        }
    }
