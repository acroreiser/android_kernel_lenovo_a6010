...
static int synaptics_t1320_power_on(struct i2c_client *client, int on)
{
	...
	if (on) {
		...
		rc = regulator_enable(vreg_l22);
		rc = regulator_enable(vreg_lvs3);
	} else {
		TOUCH_INFO_MSG("touch disable\n");
		rc = regulator_disable(vreg_l22);
		rc = regulator_disable(vreg_lvs3);
	}
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_mt_init_slots(ts->input_dev, ts->pdata->max_id, 0);
