...
#define REG_DEF_CNTRL5		(0x78)	/** CNTRL5 default value */
...
#define LSM303D_FTM_ENABLE 1
#ifdef LSM303D_FTM_ENABLE
...
static struct sensors_classdev  sensors_acc_cdev  = {
		...
		.max_range = "78.4532",	/* 16g */
		.resolution = "0.0095768",	/* 15.63mg */
		.sensor_power = "0.5",	/* typical value */
...
static struct sensors_classdev sensors_mag_cdev = {
	.name = "lsm303d-mag",
	.vendor = "st",
	.version = 1,
...
static const struct lsm303d_mag_platform_data default_lsm303d_mag_pdata = {
	...
	.max_interval = LSM303D_MAG_MAX_POLL_PERIOD_MS,
	.fs_range = LSM303D_MAG_FS_8G,
	.rot_matrix = {
		{0, 1, 0},
		{1, 0, 0},
		{0, 0, -1},
...
static int lsm303d_acc_get_ST_data(struct lsm303d_status *stat, int *xyz);
#endif
unsigned int cs_ctrl;
	...
	if (stat->use_smbus) {
		if (len == 1) {
			...
			dev_warn(&stat->client->dev,
				"lsm303d i2c_smbus_read_byte_data: ret=0x%02x, len:%d ,"
...
#ifdef DEBUG
			dev_warn(&stat->client->dev,
				"lsm303d i2c_smbus_read_i2c_block_data: ret:%d len:%d, "
				"command=0x%02x, ",
				ret, len, cmd);
			for (ii = 0; ii < len; ii++)
		...
		if (ret < 0) {
			dev_err(&stat->client->dev,
				"lsm303d read transfer error: len:%d, command=0x%02x\n",
				len, cmd);
			return 0;
		}
		return len;
...
#ifdef DEBUG
			dev_warn(&stat->client->dev,
				"lsm303d i2c_smbus_write_byte_data: ret=%d, len:%d, "
...
#ifdef DEBUG
			dev_warn(&stat->client->dev,
				"lsm303d i2c_smbus_write_i2c_block_data: ret=%d, "
...
static int lsm303d_hw_init(struct lsm303d_status *stat)
{
	...
	pr_info("lsm303d_hw_init buf[0]:%d err:%d\n", buf[0],err);
	if (err < 0) {
...
static void lsm303_cs_init(void)
{
	int r;
	pr_info("lsm303_cs_init entry\n");
	if (gpio_is_valid(cs_ctrl)) {
		r = gpio_request(cs_ctrl, "lsm_cs_gpio");
		if (r) {
			pr_info(	"unable to request gpio \n");
			goto done;
		}
		r = gpio_direction_output(cs_ctrl, 1);
		if (r) {
			pr_info(	"unable to set direction gpio \n");
			goto done;
		}
	} else {
		pr_info("gpio not provided\n");
		goto done;
	}
//	gpio_set_value(cs_ctrl, 1);	
	pr_info("lsm303_cs_init end\n");
done:
	pr_info("lsm303_cs_init fail\n");
}
...
static int lsm_acc_cdev_enable(struct sensors_classdev *sensors_cdev, unsigned int enable)
{
	struct lsm303d_status *stat = container_of(sensors_cdev, struct lsm303d_status, acc_cdev);
...
static int lsm_acc_cdev_poll_delay(struct sensors_classdev *sensors_cdev, unsigned int delay_ms)
{
	struct lsm303d_status *stat = container_of(sensors_cdev, struct lsm303d_status, acc_cdev);

	unsigned int interval_ms = delay_ms;

	interval_ms = (unsigned int)max((unsigned int)interval_ms, stat->pdata_acc->min_interval);
	mutex_lock(&stat->lock);
	stat->pdata_acc->poll_interval = (unsigned int)interval_ms;
	lsm303d_acc_update_odr(stat, interval_ms);
	mutex_unlock(&stat->lock);
...
static int lsm_mag_cdev_enable(struct sensors_classdev *sensors_cdev, unsigned int enable)
{
	struct lsm303d_status *stat = container_of(sensors_cdev, struct lsm303d_status, mag_cdev);
...
static int lsm_mag_cdev_poll_delay(struct sensors_classdev *sensors_cdev, unsigned int delay_ms)
{
	struct lsm303d_status *stat = container_of(sensors_cdev, struct lsm303d_status, mag_cdev);

	unsigned int interval_ms = delay_ms;

	interval_ms = (unsigned int)max((unsigned int)interval_ms, stat->pdata_mag->min_interval);
	interval_ms = (unsigned int)min((unsigned int)interval_ms, stat->pdata_mag->max_interval);
	mutex_lock(&stat->lock);
	stat->pdata_mag->poll_interval = (unsigned int)interval_ms;
	lsm303d_mag_update_odr(stat, interval_ms);
	mutex_unlock(&stat->lock);
...
#endif
#ifdef LSM303D_FTM_ENABLE
static ssize_t attr_get_acc_raw_data(struct kobject *kobj,struct kobj_attribute *attr,char *buf)
{
	struct device *dev = to_dev(kobj->parent);
	struct lsm303d_status *stat = dev_get_drvdata(dev);
	int databuf[3];
...
static ssize_t attr_get_acc_self_data(struct kobject *kobj,struct kobj_attribute *attr,char *buf)
{
	struct device *dev = to_dev(kobj->parent);
	struct lsm303d_status *stat = dev_get_drvdata(dev);
	s32 temp[3] ;
	s32 OUTX_NOST,OUTY_NOST,OUTZ_NOST;
	s32 OUTX_ST,OUTY_ST,OUTZ_ST;
	s32 ABS_OUTX,ABS_OUTY,ABS_OUTZ;
	int err;
	u8 databuf[2];
	u8 result = -1;

	databuf[1] = 0x00;
	databuf[0] = REG_CNTRL2_ADDR;     
	err = lsm303d_i2c_write(stat, databuf, 1);
	if (err < 0)
		dev_err(&stat->client->dev, "REG_CNTRL2_ADDR write err\n");

	databuf[1] = 0x00;
	databuf[0] = REG_CNTRL3_ADDR;
	err = lsm303d_i2c_write(stat, databuf, 1);
	if (err < 0)
		dev_err(&stat->client->dev, "REG_CNTRL3_ADDR write err\n");

	databuf[1] = 0x00;
	databuf[0] = REG_CNTRL4_ADDR;
	err = lsm303d_i2c_write(stat, databuf, 1);
	if (err < 0)
		dev_err(&stat->client->dev, "REG_CNTRL4_ADDR write err\n");

	databuf[1] = 0x5F;
	databuf[0] = REG_CNTRL1_ADDR;
	err = lsm303d_i2c_write(stat, databuf, 1);
	if (err < 0)
		dev_err(&stat->client->dev, "REG_CNTRL4_ADDR write err\n");
	
	msleep(100);	
	lsm303d_acc_get_ST_data(stat, temp);

	OUTX_NOST =  temp[0] /5000;
	...
	OUTZ_ST =  temp[2] /5000;
	 ...
	 if(((ABS_OUTX >= MIN_ST)&&(ABS_OUTX <= MAX_ST)) && ((ABS_OUTY >= MIN_ST)&&(ABS_OUTY <= MAX_ST))
	 												&& ((ABS_OUTZ >= MIN_ST)&&(ABS_OUTZ <= MAX_ST)))
	{
		pr_info(" SELF TEST SUCCESS ABS_OUTX,ABS_OUTY,ABS_OUTZ:%d ,%d,%d\n", ABS_OUTX,ABS_OUTY,ABS_OUTZ);
		result = 1;
	}
	else
	{
		pr_info(" SELF TEST FAIL ABS_OUTX,ABS_OUTY,ABS_OUTZ:%d ,%d,%d\n", ABS_OUTX,ABS_OUTY,ABS_OUTZ);
              result = 0;
	 }
	...
	err = lsm303d_i2c_write(stat, databuf, 1);
	if (err < 0)
		dev_err(&stat->client->dev, "REG_CNTRL1_ADDR write err\n");
...
static ssize_t attr_get_mag_raw_data(struct kobject *kobj,struct kobj_attribute *attr,char *buf)
{
	struct device *dev = to_dev(kobj->parent);
	struct lsm303d_status *stat = dev_get_drvdata(dev);
	int databuf[3];
	...
	mutex_lock(&stat->lock);
	lsm303d_mag_get_data(stat, databuf);
	mutex_unlock(&stat->lock);
					...
					char *buf)
{
	...
	for(i=5; i <0x40; i++ ) {
		databuf[0] = i;
			err = lsm303d_i2c_read(stat, databuf, 1);		
		if (err < 0)
		{
			pr_info("lsm303d read reg error\n");
		}
		else
		{
			pr_info("lsm303d reg:%x Val:%x\n", i, databuf[0]);
		}		
	}
...
__ATTR(gsensor_cali_x, 0664, attr_get_cali_data_x, attr_set_cali_data_x);
static struct kobj_attribute gs_cali_y =
__ATTR(gsensor_cali_y, 0664, attr_get_cali_data_y, attr_set_cali_data_y);
static struct kobj_attribute gs_cali_z =
__ATTR(gsensor_cali_z, 0664, attr_get_cali_data_z, attr_set_cali_data_z);
#endif
...
#ifdef LSM303D_FTM_ENABLE
static struct kobj_attribute gs_ftm_raw =
__ATTR(gsensor_ftm_raw, 0664, attr_get_acc_raw_data, NULL);
static struct kobj_attribute gs_ftm_self =
__ATTR(gsensor_ftm_self, 0664, attr_get_acc_self_data, NULL);
static struct kobj_attribute mag_ftm_raw =
__ATTR(msensor_ftm_raw, 0664, attr_get_mag_raw_data, NULL);
#endif
...
static int lsm303d_acc_get_ST_data(struct lsm303d_status *stat, int *xyz)
{
	...
	#ifdef DEBUG
		pr_info("%s read x=%X %X(regH regL), x=%d(dec) [ug]\n",			
			LSM303D_ACC_DEV_NAME, acc_data[1], acc_data[0], hw_d[0]);
		pr_info("%s read y=%X %X(regH regL), y=%d(dec) [ug]\n",
			LSM303D_ACC_DEV_NAME, acc_data[3], acc_data[2], hw_d[1]);
		pr_info("%s read z=%X %X(regH regL), z=%d(dec) [ug]\n",
...
static int lsm_power_on(struct lsm303d_status *data, bool on)
{
	int rc;
...
static int lsm_power_init(struct lsm303d_status *data, bool on)
{
	int rc;
		...
		const struct i2c_device_id *id)
{
	...
	err = lsm_power_init(stat, true);
	if (err) {
		dev_err(&client->dev, "power init failed");
	}
	
	err = lsm_power_on(stat, true);
	if (err) {
	...
	if(node){
		cs_ctrl = of_get_named_gpio(node, "st,cs-gpio", 0);
			if (!gpio_is_valid(cs_ctrl))
			{
				dev_err(&client->dev, "[MOMO]!!ERROR!! gpio unvalid %d\n", err);
			}
			else
			{
	...
	{
		printk("lsm303d node is not NUll\n");
	}
		lsm303_cs_init();
...
/* Register to sensors class */
	stat->acc_cdev = sensors_acc_cdev;
	stat->acc_cdev.sensors_enable = lsm_acc_cdev_enable;
	stat->acc_cdev.sensors_poll_delay = lsm_acc_cdev_poll_delay;

	stat->mag_cdev = sensors_mag_cdev;
	stat->mag_cdev.sensors_enable = lsm_mag_cdev_enable;
	stat->mag_cdev.sensors_poll_delay = lsm_mag_cdev_poll_delay;
	...
	err = sensors_classdev_register(&client->dev, &stat->mag_cdev);
	if (err) {
		pr_err("%s: Unable to register to sensors class: %d\n",
				__func__, err);
		goto exit_unregister_mag_class;
	}
...
exit_unregister_mag_class:
	sensors_classdev_unregister(&stat->mag_cdev);
exit_unregister_acc_class:
	sensors_classdev_unregister(&stat->acc_cdev);
err_input_cleanup:
...
static int lsm303d_suspend(struct i2c_client *client, pm_message_t mesg)
{
	struct lsm303d_status *stat = i2c_get_clientdata(client);
	
	pr_info("##################%s, acc, mag is %d, %d\n", __func__, stat->acc_need_resume, stat->mag_need_resume);
	if (atomic_read(&stat->enabled_acc))
	{
	...
	pr_info("lsm303d_suspend finished!!\n");	
	return 0;
}
...
static int lsm303d_resume(struct i2c_client *client)
{
	...
	pr_info("##################%s, acc, mag is %d, %d\n", __func__, stat->acc_need_resume, stat->mag_need_resume);
	if ( stat->acc_need_resume == 1 )
	{
		lsm303d_acc_enable(stat);
	}

	if ( stat->mag_need_resume == 1 )
	{
		lsm303d_mag_enable(stat);
	}
	pr_info("lsm303d_resume finished!!\n");	
	return 0;
}
#endif
...
static struct of_device_id lsm_match_table[] = {
...
static struct i2c_driver lsm303d_driver = {
	.driver = {
			...
			.of_match_table =lsm_match_table,
		  },
	.probe = lsm303d_probe,
...
static int __init lsm303d_init(void)
{
	...
	ret = i2c_add_driver(&lsm303d_driver);
	pr_info("lsm303d_init ret :%d",ret);
	return ret;
}
